...
function exportWithItemsButtonClick(){
    var ds = Ext.grid.ordersGrid.getStore();
...
function historyButtonClick(){
    var selected = Ext.grid.ordersGrid.getSelectionModel().getSelected();
...
function itemHistoryButtonClick(){
    var selected = Ext.grid.orderitemsGrid.getSelectionModel().getSelected();
    if (selected == undefined){
        Ext.Msg.show({
            title:'Error',
...
function viewNewWinButtonClick(){
    ...
    if (selections && selections.length == 1){
        var selected = selections[0];
        if (selected && selected.get("id") != null){
            window.open("order!view.bc?id="+selected.get("id"), "_blank");
        }
    } else {
        Ext.MessageBox.alert('Error', "You must select an order to view.");
    }
...
function updateTitleInventoryItemDetail(force){
    var selections = Ext.grid.titlesGrid.getSelectionModel().getSelections();
    if (selections && selections.length == 1){
        var selected = selections[0];
        if (selected && selected.get("id") != null){
            ...
            if (force || currentDetailId != selected.get("id")) {
                ...
                if (panel){
                    ...
                    params: {id:selected.get("inventoryItem_id")},
...
function updateOrderItemDetail(force){
    var selections = Ext.grid.orderitemsGrid.getSelectionModel().getSelections();
    if (selections && selections.length == 1){
        var selected = selections[0];
        if (selected && selected.get("id") != null){
...
function editNextItem(){
    if (Ext.grid.orderitemsGrid){
        var selModel = Ext.grid.orderitemsGrid.getSelectionModel();
        if (!selModel.hasNext()){
            Ext.Msg.show({
                title:'Error',
...
function deleteItemButtonClick(){
    if (Ext.grid.orderitemsGrid){
        var selected = Ext.grid.orderitemsGrid.getSelectionModel().getSelections();
        if (selected == undefined){
            Ext.Msg.show({
                title:'Error',
...
function printInvItemNewWindowButtonClick(){
    if (Ext.grid.orderitemsGrid){
        var selected = Ext.grid.orderitemsGrid.getSelectionModel().getSelected();
        if (selected == undefined){
            Ext.Msg.show({
        ...
        showPrintInv(selected.get("inventoryItem_id"));
    }    
}
    ...
    if (selections && selections.length == 1){
        ...
        if (selected && selected.get("id") != null){
            ...
            Ext.crudWindow = new Ext.Window({
                ...
                bbar:[],
                bodyStyle:'background-color:#fbfbfb',
                autoLoad: {url:'orderitem!shipped.bc?id='+selected.get('id'), params: Ext.grid.orderitemsGridDs.lastOptions.params}
            });
            Ext.crudWindow.show(Ext.grid.orderitemsGrid);
        }
    }
...
function unpostButtonClick(){
    if (Ext.grid.ordersGrid){
        ...
        if (selected == undefined){
            Ext.Msg.show({
                title:'Error',
                msg: 'You must select an Order to unpost.',
                buttons: Ext.Msg.OK
                });
            return;
        }
        if (selected.get('posted') == false){
            Ext.Msg.show({
                title:'Error',
                msg: 'This order is already unposted.',
                buttons: Ext.Msg.OK
                });
            return;
        }
