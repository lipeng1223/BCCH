...
<inv:table tableName="belltitles" sortable="true" exportable="true" tableTitle="Title History"
dataAction="history!titleData.bc?id=${id}" table="${listTable}" addToContainer="listpanel" stateful="false"/>

</div>

<script language="JavaScript" type="text/javascript">
    Ext.onReady(function(){
        Ext.grid.belltitlesGrid.getSelectionModel().on('selectionchange', function(){
            setTimeout("updateTitleInventoryItemDetail()", 300);
        });
        Ext.grid.beltitlesGridDs.on('load', function(store, records, options){
