...
<title><s:property value="order.bellCustomer.code"/><s:if test="order.bellCustomer.code != null && order.bellCustomer.code.length() > 0 && order.bellCustomer.companyName != null && order.bellCustomer.companyName.length() > 0"> - </s:if><s:property value="order.bellCustomer.companyName"/></title>
...
function importItemButtonClick(){
    ...
    Ext.uploadWindow.show(Ext.grid.bellorderitemsGrid);
}
...
<s:if test="isBcOrderAdmin && order.posted == false">
    <inv:table tableName="bellorderitems" sortable="true" exportable="true" tableTitle="Order Items"
...
<script language="JavaScript" type="text/javascript">
Ext.onReady(function(){
    Ext.grid.bellorderitemsGrid.getSelectionModel().addListener("rowselect", function(model, rowindex, rec){
...
<script language="JavaScript" type="text/javascript">
Ext.onReady(function(){
    Ext.grid.bellorderitemsGridDs.on("load", function(store, records, opts){
        if (doEditNextItem != null){
            var selId = doEditNextItem;
            doEditNextItem = null;
            var foundRec;
            Ext.grid.bellorderitemsGridDs.each(function(rec){
               if (rec.get("id") == selId){
                   foundRec = rec;
                   return false;
               } 
            ...
            if (foundRec != undefined){
                var idx = Ext.grid.bellorderitemsGridDs.indexOf(foundRec);
                Ext.grid.bellorderitemsGrid.getSelectionModel().selectRow(idx);
                editNextItem();
            }
        }
    });
