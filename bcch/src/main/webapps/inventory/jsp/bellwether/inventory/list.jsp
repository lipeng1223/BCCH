...
<s:set name="stateSession" value="%{'bell-inventory-list-state'}"/>
...
<inv:table tableName="bellinventory" sortable="true" exportable="true" tableTitle="Inventory Items By ISBN"
dataAction="inventory!listData.bc" table="${listTable}" addToContainer="listpanel" stateful="true"
rowDblClick="rowDoubleClick" defaultFilters="${defaultFilters}"/>
</s:if>
<s:elseif test="isQuickSearch">
<inv:table tableName="bellinventory" sortable="true" exportable="true" tableTitle="Inventory Items - Quick Search - ${quickSearch}"
dataAction="inventory!quickSearchListData.bc?quickSearch=${quickSearch}" table="${listTable}" addToContainer="listpanel" stateful="true"
...
<script language="JavaScript" type="text/javascript">
    Ext.onReady(function(){
        Ext.grid.bellinventoryGrid.getSelectionModel().on('selectionchange', function(){
            setTimeout("updateInventoryDetail()", 300);
        });
        Ext.grid.bellinventoryGridDs.on('load', function(store, records, options){
