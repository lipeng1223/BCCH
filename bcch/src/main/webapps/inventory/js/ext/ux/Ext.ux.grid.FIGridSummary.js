Ext.ns("Ext.ux.grid.FIGridSummary"); // namespace Ext.ux.grid.GridSummary

Ext.ux.grid.FIGridSummary = function(config) {
    Ext.apply(this, config);
};

Ext.extend(Ext.ux.grid.FIGridSummary, Ext.util.Observable, {
  init : function(grid) {
    this.grid = grid;
    ...
    for (var j = 0, jlen = rs.length; j < jlen; j++) {
      ...
      for (var i = 0, len = cs.length; i < len; i++) {
        ...
        if (cf && cf.summaryType) {
          data[c.name] = Ext.ux.grid.FIGridSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data);
        }
      }
    }
  ...
  updateSummary : function(summary){
    isXml = Ext.type(summary) == 'element';
    var cs = this.view.getColumnData();
    for (var i = 0, len = cs.length; i < len; i++) {
      c = cs[i];
      var v = isXml ? Ext.DomQuery.selectValue(c.name, summary) : summary[c.name];
      if (v) {
        this.cols[i] = v;
      }
      else this.cols[i] = '';
  ...
  renderSummary : function(o, cs) {
    ...
    for (var i = 0, len = cs.length; i < len; i++) {
      ...
      if (cf.summaryRenderer) {
          p.value = (cf.summaryRenderer || c.renderer)(this.cols[i], p, o);
      } else {
          p.value = this.cols[i];
      }
