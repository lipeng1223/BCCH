...
Ext.extend(Ext.ux.grid.GridFilters, Ext.util.Observable, {
	...
	defaultFilter: null,
	defaultFilterCol: null,
	doDefaultFilter: false,
    defaultFilters: null,
	...
	init: function(grid){
		if(grid instanceof Ext.grid.GridPanel){
			...
			if (this.doDefaultFilter){
			    if (this.defaultFilters != null && this.defaultFilters['filters'] != undefined) {
			        for (var i = 0; i < this.defaultFilters['filters'].length; i++){
			            var filter = this.defaultFilters['filters'][i];
	                    var gf = this.filters.get(filter['col']);
			            if (gf != undefined){
			                if (filter['val']['before'] != undefined){
			                    filter['val']['before'] = new Date(filter['val']['before']);
			                } 
			                if (filter['val']['after'] != undefined){
                                filter['val']['after'] = new Date(filter['val']['after']);
                            }
			                if (filter['val']['on'] != undefined){
                                filter['val']['on'] = new Date(filter['val']['on']);
			                }
	                        gf.setValue(filter['val']);
	                        gf.setActive(true, true);
			            }
			        }
			        this.doDefaultFilter = false;
			    } else {
	                var filter = this.filters.get(this.defaultFilterCol);
	                if(filter){
	                    filter.setValue(this.defaultFilter);
	...
	onStateChange: function(event, filter){
		...
		else if(event == "activate" || event == "update" || event == "deactivate"){
            this.reload();
            if (this.grid.filterRow != undefined){
                var frow = this.grid.filterRow.getFilterField(filter.dataIndex);
                if (frow != undefined){
                    if (event == "deactivate")
                        frow.filter.setFieldValue("");
                    else
                        frow.filter.setFieldValue(filter.getValue());
                }
            }
        }
	...
	/** private **/
	reload: function(){
	    //console.log("reload, doDefaultFilter: "+this.doDefaultFilter);
		if (!this.doDefaultFilter){
			if(this.local){
