...
Ext.ux.grid.FilterRow = Ext.extend(Ext.util.Observable, {
  ...
  init: function(grid) {
    ...
    this.eachFilterColumn(function(col) {
      if (!(col.filter instanceof Filter)) {
        col.filter = new Filter(col.filter, col.dataIndex, this.gridFilter);
      }
      col.filter.on("keypress", this.onKeyPress, this);
    });
  ...
  getFilterField: function(di){
      theFilter = undefined;
      if (this.grid == undefined || this.grid.getColumnModel() == undefined) return undefined;
      this.eachFilterColumn(function(col) {
          //console.log(col.filter.dataIndex+" "+di);
          if (col.filter.dataIndex == di){
              theFilter = col;
          }
      });
      return theFilter;
  ...
  onKeyPress: function(field, evt){
      //console.log("*********************************   onKeyPress");
      //console.log(inspect(evt));
      //console.log(inspect(field));
  },
...
Ext.ux.grid.FilterRowFilter = Ext.extend(Ext.util.Observable, {
  ...
  constructor: function(config, di, gf) {
  ...
  fireChangeEvent: function(field, evt) {
      //console.log("----- fireChangeEvent");
      //console.log(inspect(evt));
      //console.log(evt.getKey());
      //console.log(evt.getKey() == evt.ENTER);
      if (evt.getKey() == evt.ENTER) {
          //console.log(this.dataIndex);
          //console.log(this.getFieldValue());
          var filter = this.gridFilter.getFilter(this.dataIndex);
          if (filter != undefined){
              filter.setValue(this.getFieldValue());
              if (this.getFieldValue().length > 0) {
                  filter.setActive(true, false);
              } else {
                  filter.setActive(false, false);
              }
          }
